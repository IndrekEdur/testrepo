...
from . models import Product, Project, Comment, Hours, UserData, Remark
from . forms import ProductForm, CommentForm, HoursForm, GeneralHoursForm, ProjectForm, RemarkForm
...
def ShowAllProjects(request):
    # Determine the current sorting order and toggle it
    current_sort = request.GET.get('sort', 'project_number')
    new_sort = '-project_number' if current_sort == 'project_number' else 'project_number'

    # Sort the projects
    projects = Project.objects.order_by(current_sort)
    context = {'projects': projects, 'new_sort': new_sort}
        ...
        if request.method == 'POST':
            ...
            if xlsx_file:
                ...
                for row in sheet.iter_rows(min_row=2, values_only=True):
                    # Assuming columns: name, is_active, project_number
                ...
                messages.success(request, 'Projects imported successfully.')
                return redirect('products/project_list.html')
            else:
                messages.error(request, 'No file was selected.')

        # Render or redirect as appropriate
        return render(request, 'products/project_import.html')


@login_required(login_url='accounts/login.html')
def ShowAllRemarks(request):
    remarks = Remark.objects.order_by('-date_added')
    context = {'remarks': remarks}
    return render(request, 'products/remark_list.html', context)
...
def addRemark(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('showRemarks')
    else:
        form = RemarkForm()
    context = {
        "form": form
     }
    return render(request, 'products/addRemark.html', context)
...
def WorkingHoursListView(request, pk):
    ...
    years = ["2023", "2022"]
    hours_exist_in_month = 0
    context = { "hours_exist_in_month": hours_exist_in_month, "years": years, "months": months, "hours": hours, "product": Product.objects.all, "sum": hours_sum}
